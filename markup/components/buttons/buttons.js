    ...
    function _desktopDrawButtons(stage, loader) {
        ...
        spinBtn.on('click', () => {
            if (spinBtn.currentAnimation === 'spinBtn'
                || spinBtn.currentAnimation === 'spinBtnOn'
            ) {
                // if (isRoll) return;
                handleSpinClick();
        ...
        autoSpinBtn.on('click', () => {
            if (isRoll) return;
            if ( storage.read('isFreeSpin') ) return;
        ...
        staticBtnContainer.addChild(home, soundBtn, fastSpinBtn);
        // staticBtnContainer.addChild(settingsBtn);
